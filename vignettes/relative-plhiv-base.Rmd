 ...
 #limit Test and Treat data
  df_tt_plhiv <- df_tt %>% 
    filter(year == max(year),
           country == "Namibia",
           indicator %in% c("Percent Known Status of PLHIV", "Percent on ART of PLHIV", "Percent VLS of PLHIV"), #Using the PLHIV base indicators
           age == "All",
           sex == "All") %>% 
...
knitr::kable(df_tt_plhiv,
             format = "html",
             caption = "Namibia's 95-95-95 Progress using PLHIV Base") %>% 
...
goal <- 95
 ...
 #limit Test and Treat data
  df_tt_rel <- df_tt %>% 
    filter(year == max(year),
           country == "Namibia",
           indicator %in% c("Percent Known Status of PLHIV", "Percent on ART with Known Status", "Percent VLS on ART"), #using relative base indicators
           age == "All",
           sex == "All") %>%
...
knitr::kable(df_tt_rel,
             format = "html",
             caption = "Namibia's 95-95-95 Progress using the Relative Base") %>% 
    kable_styling(bootstrap_options = c("hover"))
...
```{r, echo = FALSE}
knitr::include_graphics("5-countries-relative-base.png", dpi = 150)
```

With the PLHIV base, the list of countries expands from 5 to 6 countries that have reached all 3 95's, adding **Namibia**.

```{r, echo = FALSE}
knitr::include_graphics("6-countries-PLHIV-base.png", dpi = 150)
```
