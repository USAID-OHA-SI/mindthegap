   ...
   year <- unique(df_tt_rel$year)
      ...
      gt(df_tt_rel) %>% 
	      ...
	      tab_options(source_notes.font.size = px(8),
	                  data_row.padding = px(1),
	                  table.font.size = px(12)) %>% 
	      tab_header(title ="NAMIBIA'S {year} TREATMENT TARGET GOALS: RELATIVE BASE") %>% 
      gt_color_rows(achieved,
	                    palette = c(glitr::burnt_sienna, glitr::scooter),
	                    domain = c(0,1),
	                    pal_type = "discrete")
...
With the relative base and PLHIV base, 7 countries have reached all 3 95's: **Botswana, Eswatini, Kenya, Malawi, Rwanda, Zambia, and Zimbabwe**.
