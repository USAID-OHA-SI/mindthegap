...
This vignette covers how to access the clean and tidy annual UNAIDS Estimates from Google Drive to R. The MindTheGap package uses a cleaning function (`munge_unaids`) to process, munge, and tidy the UNAIDS HIV Estimates from AIDSInfo and `pull_unaids` to pull the cleaned data from Google Drive.
...
The data structure itself is not tidy and requires some munging and reshaping before it can be used for analytics and visualizations. As such, the `munge_unaids` function reads in the original dataset from Google Drive, munges and reshapes the data into a tidy format, and exports the data as a dataframe.
...
1. `sheetname`: This parameter corresponds to the earlier parameter in munge_unaids that denotes the return_type and indicator_type. For this parameter, use “HIV Estimates - Integer”, “HIV Estimates - Percent", "Test & Treat - Integer", "Test & Treat - Percent". 
2. `pepfar_only`: if TRUE, this parameter will return a dataset of only PEPFAR countries 

In order to access the data from Google drive, be sure to load your USAID credentials. You can achieve this by using the `glamr::load_secrets()` in the `glamr` package or authenticate through the `googlesheets` package using `googlesheets::gs_auth()`.
